...
def home():
    ...
    if 'place_id' in session:
       ...
       if request.method =="POST" and "search_name" in request.form:      
           v_name = request.form['search_name']
           cur=mysql_create_connection()
           cur.execute(f"SELECT visitor_name, visitor_address, email, phone_number, infected from Visitor V WHERE V.visitor_name LIKE '%{v_name}%' OR V.visitor_address LIKE '%{v_name}%'")
           visitors = cur.fetchall()
           count = cur.rowcount
           if len(visitors)==0 and len(v_name)==0:
               cur = mysql_create_connection()
               cur.execute(
       ...
       #visitors = cur.fetchall()
       #count = cur.rowcount
       return render_template('Home.html', loggedIn=True, agent_id=agent_id, headings=headings)
...
@app.route('/scan/owner', methods=['GET', 'POST'])
@auto.doc()
def qrcode_success():
