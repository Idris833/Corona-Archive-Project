...
class FlaskTestCase(unittest.TestCase):
    ...
    # First run: Working Test Cases - 18 . Failed Test Cases 2
    ...
    def test_login_hospital_account_success(self):
        tester = app.test_client(self)
        response = tester.post(
            '/authenticate', data=dict(hospitalUsername="Melina", hpass="12345asd"), follow_redirects=True)
        self.assertIn(b'Hospital\'s Home Page', response.data)

    def test_login_agent_account_success(self):
        tester = app.test_client(self)
        response = tester.post(
            '/authenticate', data=dict(agentUsername="John", apass="asdas1221"), follow_redirects=True)
    ...
    # The success page opens successfully
    def test_success_page(self):
        tester = app.test_client(self)
        response = tester.get('/success2', content_type="html/text")
        self.assertIn(b'Success Page', response.data)
